defmodule FenceAlchemyWeb.CoreComponents do
  @moduledoc """
  ...
  use Gettext, backend: FenceAlchemyWeb.Gettext
  ...
  def button(assigns) do
    ...
    <button
      type={@type}
      class={[
        "phx-submit-loading:opacity-75 rounded-lg bg-yellow-700 hover:bg-yellow-800 py-2 px-3",
          ...
          <tr :for={row <- @rows} id={@row_id && @row_id.(row)} class="group hover:bg-zinc-50">
            ...
            <td :if={@action != []} class="relative w-14 p-0">
              <div class="relative whitespace-nowrap py-4 text-right text-sm font-medium">
                ...
                <span
                  :for={action <- @action}
                  class="relative ml-4 font-semibold leading-6 text-white hover:text-white"
                >
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(FenceAlchemyWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(FenceAlchemyWeb.Gettext, "errors", msg, opts)
    end
  end
